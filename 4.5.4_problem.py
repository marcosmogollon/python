...
def modify_dict(my_dictionary):
    ...
    for key in my_dictionary:
        # for each key in the dictionary, check the first letter (each ley is s tring so string_name[0] is the first letter in each string)
        if not key[0].isupper():
            # add each key we want to remove to a list
            delete_list.append(key)
        else:
            #if the frist letter is a capital then keep going
            continue
    # for each item in the list delete the corresponding key in the dictionary
