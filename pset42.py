...
def find_pressure(num_of_moles, temperature, volume, R = 0.082057):
    num_of_moles = float(num_of_moles)
    temperature = float(temperature)
    volume = float(volume)
    try:
        end_result = (num_of_moles * R * temperature) / volume
    except:
        end_result = "Volume must be greater than 0."
    return end_result
